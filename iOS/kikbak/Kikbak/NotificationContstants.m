//
//  NotificationContstants.m
//  Kikbak
//
//  Created by Ian Barile on 4/25/13.
//  Copyright (c) 2013 Ian Barile. All rights reserved.
//

#import "NotificationContstants.h"

NSString *const kKikbakLocationUpdate = @"kKikbakLocationUpdate";
NSString *const kKikbakImageDownloaded = @"kKikbakImageDownloaded";
NSString *const kKikbakOffersDownloaded = @"kKikbakOffersDownloaded";
NSString *const kKikbakRewardUpdate = @"kKikbakRewardUpdate";
NSString *const kKikbakRedeemGiftError = @"kKikbakRedeemGiftError";
NSString *const kKikbakRedeemGiftSuccess = @"kKikbakRedeemGiftSuccess";
NSString *const kKikbakRedeemCreditError = @"kKikbakRedeemCreditError";
NSString *const kKikbakRedeemCreditSuccess = @"kKikbakRedeemCreditSuccess";
NSString *const kKikbakShareSuccess = @"kKikbakShareSuccess";
NSString *const kKikbakShareError = @"kKikbakShareError";
NSString *const kKikbakClaimSuccess = @"kKikbakClaimSuccess";
NSString *const kKikbakClaimError = @"kKikbakClaimError";
NSString *const kKikbakBarcodeSuccess = @"kKikbakBarcodeSuccess";
NSString *const kKikbakBarcodeError = @"kKikbakBarcodeError";
NSString *const kKikbakImagePostSuccess = @"kKikbakImagePostSuccess";
NSString *const kKikbakImagePostError = @"kKikbakImagePostError";
NSString *const kKikbakFBStoryPostSuccess = @"kKikbakFBStoryPostSuccess";
NSString *const kKikbakFBStoryPostError = @"kKikbakFBStoryPostError";
NSString *const kKikbakQrcodeSuccess = @"kKikbakQrcodeSuccess";
NSString *const kKikbakQrcodeError = @"kKikbakQrcodeError";