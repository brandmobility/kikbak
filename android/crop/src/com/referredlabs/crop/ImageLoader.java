...
public final class ImageLoader {
  ...
  /**
   * Returns the Mime type for a Url. Safe to use with Urls that do not come from Gallery's content
   * provider.
   */
  ...
  /**
   * Returns the image's orientation flag. Defaults to ORI_NORMAL if no valid orientation was found.
   */
  public static int getMetadataOrientation(Context context, Uri uri) {
    ...
    if (ContentResolver.SCHEME_FILE.equals(uri.getScheme())) {
      ...
      try {
        ...
        return exif.getAttributeInt(ExifInterface.TAG_ORIENTATION, ORI_NORMAL);
      } catch (IOException e) {
        return ORI_NORMAL;
      }
    }
  ...
  /**
   * Returns the rotation of image at the given URI as one of 0, 90, 180, 270. Defaults to 0.
   */
  ...
  /**
   * Takes an orientation and a bitmap, and returns the bitmap transformed to that orientation.
   */
  ...
  /**
   * Loads a bitmap at a given URI that is downsampled so that both sides are smaller than
   * maxSideLength. The Bitmap's original dimensions are stored in the rect originalBounds.
   * 
  ...
  /**
   * Loads a bitmap at a given URI that is downsampled so that both sides are smaller than
   * maxSideLength. The Bitmap's original dimensions are stored in the rect originalBounds. The
   * output is also transformed to the given orientation.
   * 
   * @param uri URI of image to open.
   * @param context context whose ContentResolver to use.
   * @param maxSideLength max side length of returned bitmap.
  ...
  /**
   * Loads a bitmap that is downsampled by at least the input sample size. In low-memory situations,
   * the bitmap may be downsampled further.
   */
  ...
  /**
   * Loads an oriented bitmap that is downsampled by at least the input sample size. In low-memory
   * situations, the bitmap may be downsampled further.
   */
